...
class entropyVector_MixedState:
    ...
    def __add__(self, other):
        if not isinstance(other, entropyVector_MixedState):
            raise Exception("Cannot add mixed state entropy vector and {}".format(other))
        
        (combState, combLabels) = entVecAdd_MixedState(self, other)
    ...
    def __mul__(self, other):
        if not (isinstance(other, int)):
            raise Exception("Cannot multiply an entropy vector and {}".format(other))
        if not (other > 0):
            raise Exception("Cannot multiply an entropy vector by {} (a non-positive integer)".format(other))
        
        (scaledState, scaledLabels) = entVecScalarMult_MixedState(entVec=self, scalar=other)        
        return(entropyVector_MixedState(densityMat=scaledState, bitPartitions=scaledLabels))
...
def entVecAdd_MixedState(entVec1: entropyVector_MixedState, entVec2: entropyVector_MixedState):
    labelArray1 = bitmap2Array(entVec1.subSystemLabels)
    labelArray2 = bitmap2Array(entVec2.subSystemLabels)
    combinedLabels = array2Bitmap(labelArray1 + labelArray2)
    combinedPureState = np.kron(entVec1.densityMatrix, entVec2.densityMatrix)
    return(combinedPureState, combinedLabels)
...
def entVecScalarMult_MixedState(entVec: entropyVector_MixedState, scalar: int):
    if not(scalar > 0):
        raise Exception("The scalar is {}, which is not > 0".format(scalar))
    origState = entVec.densityMatrix
    origLabels = entVec.subSystemLabels
    origLabelArray = bitmap2Array(origLabels)
    updatedLabels = array2Bitmap(scalar * origLabelArray)
    curEntVec = copy.deepcopy(entVec)
    curState = curEntVec.densityMatrix
    for i in range(scalar-1):
        curState = np.kron(curState, origState)
    return(curState, updatedLabels)
